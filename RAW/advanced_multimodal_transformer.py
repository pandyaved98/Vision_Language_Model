...
import math
from advanced_vision_transformer import SiglipVisionConfig, SiglipVisionModel
...
class PaliGemmaConditionalGeneration(nn.Module):
    ...
    def _merge_input_ids_with_image_features(self, image_features = torch.Tensor, input_embeds: torch.Tensor, input_ids: torch.Tensor, attention_mask: torch.Tensor, kv_cache: Optional[KVCache] = None):
        ...
        final_embedding = torch.zeros(batch_size, sequence_length, embed_dim, dtype = input_embeds.dtype, device = input_embeds.dtype, device = input_embeds.device)
    ...
    def forward(self, input_ids: torch.LongTensor = None, pixel_values: torch.FloatTensor = None, attention_mask: Optional[torch.Tensor] = None, kv_cache: Optional[KVCache] = None) -> Tuple:
        ...
        input_embeds, attention_mask, position_ids = self._merge_input_ids_with)image_features(image_features, input_embeds, input_ids, attention_mask, kv_cache)
